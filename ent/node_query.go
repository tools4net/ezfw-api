...
func (nq *NodeQuery) QueryServiceInstances() *ServiceInstanceQuery {
	query := (&ServiceInstanceClient{config: nq.config}).Query()
...
func (nq *NodeQuery) QueryAgentTokens() *AgentTokenQuery {
	query := (&AgentTokenClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NodeQuery) Clone() *NodeQuery {
	...
	return &NodeQuery{
		...
		withServiceInstances: nq.withServiceInstances.Clone(),
		withAgentTokens:      nq.withAgentTokens.Clone(),
...
func (nq *NodeQuery) WithServiceInstances(opts ...func(*ServiceInstanceQuery)) *NodeQuery {
	query := (&ServiceInstanceClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withServiceInstances = query
	return nq
}
...
func (nq *NodeQuery) WithAgentTokens(opts ...func(*AgentTokenQuery)) *NodeQuery {
	query := (&AgentTokenClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withAgentTokens = query
	return nq
}
...
func (nq *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	var (
		...
		_spec       = nq.querySpec()
		loadedTypes = [2]bool{
			nq.withServiceInstances != nil,
			nq.withAgentTokens != nil,
		}
	)
	...
	if query := nq.withServiceInstances; query != nil {
		if err := nq.loadServiceInstances(ctx, query, nodes,
			func(n *Node) { n.Edges.ServiceInstances = []*ServiceInstance{} },
			func(n *Node, e *ServiceInstance) { n.Edges.ServiceInstances = append(n.Edges.ServiceInstances, e) }); err != nil {
			return nil, err
		}
	}
	if query := nq.withAgentTokens; query != nil {
		if err := nq.loadAgentTokens(ctx, query, nodes,
			func(n *Node) { n.Edges.AgentTokens = []*AgentToken{} },
			func(n *Node, e *AgentToken) { n.Edges.AgentTokens = append(n.Edges.AgentTokens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NodeQuery) loadServiceInstances(ctx context.Context, query *ServiceInstanceQuery, nodes []*Node, init func(*Node), assign func(*Node, *ServiceInstance)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(serviceinstance.FieldNodeID)
	}
	query.Where(predicate.ServiceInstance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.ServiceInstancesColumn), fks...))
	}))
...
func (nq *NodeQuery) loadAgentTokens(ctx context.Context, query *AgentTokenQuery, nodes []*Node, init func(*Node), assign func(*Node, *AgentToken)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Node)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(agenttoken.FieldNodeID)
	}
	query.Where(predicate.AgentToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.AgentTokensColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
